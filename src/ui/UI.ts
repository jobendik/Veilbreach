...
export class UI implements GameHost {
      ...
      onCardSelected: (uid, mode) => {
        switch (mode) {
          ...
          case "remove-reward":
            ...
            break;
          case "upgrade-event":
            // Legacy path: rest-site upgrades live on RestSystem now.
            this.game.rest.finishUpgrade(uid);
            break;
          case "remove-event":
            // No event removes cards any more; closing is a no-op.
            this.closeModal();
            break;
          case "upgrade-rest":
            this.game.rest.finishUpgrade(uid);
  ...
  onReward(tier: import("../core/Types").NodeType): void {
    this.game.reward.createReward(tier);
  }
